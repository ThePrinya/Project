...
import { CheckCircle, Clock, XCircle, BookOpen, Upload, FileText, X, Edit } from 'lucide-react'; 
...
const allActivityData = {
  term2: [
    { id: 1, title: 'งานเจ๊สมร อมรรัตนโกสินทร์', subject: 'วิชาการจัดการทั่วไป', status: 'Submitted', color: 'text-blue-600', icon: CheckCircle, dueDate: '15 Feb 2024' },
    { id: 2, title: 'งาน Gen ที่ติด F', subject: 'General Education', status: 'NotSubmitted', color: 'text-red-500', icon: XCircle, dueDate: '28 Feb 2024' },
    { id: 3, title: 'งานจารย์พร', subject: 'วิชาภาษาอังกฤษพื้นฐาน', status: 'NotSubmitted', color: 'text-red-500', icon: XCircle, dueDate: '01 Mar 2024' },
    { id: 4, title: 'งานพี่สมชาย', subject: 'ปฏิบัติการคอมพิวเตอร์', status: 'LateSubmit', color: 'text-yellow-500', icon: Clock, dueDate: '10 Feb 2024' },
    { id: 5, title: 'โปรเจกต์จบ (บทที่ 1-3)', subject: 'Senior Project', status: 'Submitted', color: 'text-blue-600', icon: CheckCircle, dueDate: '20 Jan 2024' },
  ],
  term1: [
    { id: 6, title: 'แบบฝึกหัดท้ายบทที่ 1', subject: 'Calculus I', status: 'Submitted', color: 'text-blue-600', icon: CheckCircle, dueDate: '15 Aug 2023' },
    { id: 7, title: 'รายงานกลุ่ม', subject: 'Physics', status: 'Submitted', color: 'text-blue-600', icon: CheckCircle, dueDate: '30 Sep 2023' },
  ],
};

const statusFilters = [
  { label: 'ทั้งหมด', value: 'All', colorClass: 'text-gray-800', icon: BookOpen },
  { label: 'ส่งแล้ว', value: 'Submitted', colorClass: 'text-blue-600', icon: CheckCircle }, 
  { label: 'ส่งช้า', value: 'LateSubmit', colorClass: 'text-yellow-600', icon: Clock },  
  { label: 'ยังไม่ส่ง', value: 'NotSubmitted', colorClass: 'text-red-600', icon: XCircle },     
];
...
// =========================================================================================
// 🪟 [4] หน้าต่าง Pop-up รายละเอียดงาน (Task Modal)
// =========================================================================================
const TaskModal = ({ isVisible, onClose, activity }) => {
  // --- 🌟 ระบบจัดกึ่งกลางอัจฉริยะ (Dynamic Centering) ---
  // คอยวัดขนาดและตำแหน่งของ "พื้นที่เนื้อหาที่เหลือ" แบบ Real-time เพื่อให้ Pop-up ตรงกลางเสมอ
  useEffect(() => {
    if (!isVisible) return;
    const root = document.getElementById('activity-page-root');
    const wrapper = document.getElementById('modal-centering-wrapper');
  ...
  const isNotSubmitted = activity.status === 'NotSubmitted';

  return (
    // 🛑 Container ชั้นนอกสุด ครอบเต็มหน้าจอ (Viewport)
    <div className="fixed inset-0 z-[100] pointer-events-none">
      
      {/* 1. แผ่นหลังสีดำเบลอ (คลุมเต็มหน้าจอเสมอ) */}
      <div 
        className="absolute inset-0 bg-black/60 backdrop-blur-sm pointer-events-auto transition-opacity" 
        onClick={onClose}
      ></div>

      {/* 2. กรอบวัดระยะ (Dynamic Wrapper) 
          จะถูกสคริปต์ด้านบนปรับความกว้างและตำแหน่ง (Left) ให้เท่ากับพื้นที่ว่างโดยอัตโนมัติ */}
      <div 
        id="modal-centering-wrapper" 
        className="absolute inset-y-0 flex items-center justify-center p-3 sm:p-4 pointer-events-none"
      >
        
        {/* 3. กล่อง Pop-up สีขาว */}
        {/* 🛑 แก้ปัญหาในมือถือ: เปลี่ยนให้กล่องหลัก overflow-hidden แล้วสร้าง wrapper ข้างในให้ Scroll หน้าเดียว */}
        <div className="relative bg-white w-full max-w-4xl rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden transform transition-all animate-fadeIn max-h-[92vh] lg:max-h-[90vh] pointer-events-auto">
          ...
          {/* ปุ่มกากบาท (ปิด Pop-up) ยึดติดอยู่มุมขวาบนเสมอ */}
          <button 
            onClick={onClose} 
            className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-red-500 transition-colors bg-white/90 rounded-full p-2 z-50 shadow-sm border border-gray-100"
          >
            <X className="w-5 h-5 stroke-[2.5]" />
          </button>

          {/* 🛑 พื้นที่ Scroll 
              - บนมือถือ: จะรวมคอลัมน์ซ้ายขวาไหลยาวเป็นหน้าเดียว (overflow-y-auto)
              - บนจอใหญ่ (lg): จะไม่ Scroll ที่นี่ (overflow-hidden) แต่ไป Scroll แยกซ้ายขวาแทน */}
          <div className="flex flex-col lg:flex-row w-full h-full overflow-y-auto lg:overflow-hidden custom-scrollbar">
            ...
            {/* ปรับเป็น h-auto บนมือถือ และ h-full บนจอใหญ่ */}
            <div className="w-full lg:w-1/2 bg-blue-50/40 p-5 sm:p-8 lg:p-10 flex flex-col gap-4 sm:gap-6 overflow-visible lg:overflow-y-auto custom-scrollbar border-b lg:border-b-0 lg:border-r border-gray-100 h-auto lg:h-full">
              ...
              <div className="bg-white rounded-xl p-4 sm:p-5 border border-blue-100 shadow-sm space-y-3 sm:space-y-4">
                <div className="flex flex-row items-baseline sm:items-center gap-3">
                   ...
                   <span className={`text-xs lg:text-sm font-bold ${activity.color} flex items-center gap-1.5 flex-wrap`}>
                     ...
                     {activity.status === 'Submitted' ? 'ส่งแล้ว' : activity.status === 'LateSubmit' ? 'ส่งล่าช้า' : 'ยังไม่ส่ง'}
                   </span>
                </div>
                <div className="flex flex-row items-baseline sm:items-center gap-3">
              ...
              <div className="mb-2 lg:mb-0 flex-1">
                ...
                <div className="text-xs lg:text-sm text-gray-600 bg-white/60 p-4 rounded-xl border border-gray-100 min-h-[60px] lg:min-h-[80px] leading-relaxed break-words">
                  อาจารย์ได้สั่งงานให้ทำสรุปเนื้อหาบทที่ 1-3 พร้อมแนบไฟล์ PDF ส่งในระบบก่อนเวลาที่กำหนด (ข้อมูลตัวอย่างสำหรับการทดสอบ)
                </div>
              </div>
            </div>

            {/* --- ฝั่งขวา: ส่วนสำหรับการส่งงาน --- */}
            {/* ปรับเป็น h-auto บนมือถือ และ h-full บนจอใหญ่ */}
            <div className="w-full lg:w-1/2 bg-white p-5 sm:p-8 lg:p-10 flex flex-col gap-5 sm:gap-6 overflow-visible lg:overflow-y-auto custom-scrollbar h-auto lg:h-full">
              <h3 className="text-lg lg:text-xl font-bold text-[#1F384C] mb-1">การส่งงานของคุณ</h3>
              ...
              {isNotSubmitted ? (
                <div className="flex-1 flex flex-col gap-4">
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50 hover:bg-blue-50/30 hover:border-blue-300 transition-all flex flex-col items-center justify-center p-6 cursor-pointer flex-grow min-h-[140px] group">
                    ...
                    <p className="text-sm font-bold text-gray-700 text-center">ลากไฟล์มาวางที่นี่</p>
                    <p className="text-[10px] lg:text-xs text-gray-400 mt-1 text-center">PDF, Word, Image (ไม่เกิน 10MB)</p>
                  </div>
                ...
                <div className="flex-1 flex flex-col gap-4">
                  <div className="bg-blue-50/30 rounded-xl p-3 sm:p-4 border border-blue-100 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-bold text-gray-800 truncate">Work_Assignment_Final_V1.pdf</p>
                      <p className="text-[10px] sm:text-xs text-gray-500 truncate">2.4 MB • อัปโหลดเมื่อ 12 Feb 2024</p>
                    </div>
                  </div>
                  
                  <div className="mt-auto pt-4 flex flex-col gap-3">
                    <div className="text-[10px] sm:text-xs text-gray-400 text-center italic">ส่งงานเมื่อ: January 22, 2025 At 12:30</div>
                    <button className="bg-yellow-500 text-white py-3.5 sm:py-4 rounded-xl font-bold shadow-md hover:bg-yellow-600 transition-all w-full flex items-center justify-center gap-2 text-sm sm:text-base">
                      <Edit className="w-4 h-4" /> แก้ไขการส่งงาน
                    </button>
                  </div>
                </div>
              )}
...
// =========================================================================================
// 📇 [5] การ์ดรายการกิจกรรม (Activity Card)
// =========================================================================================
...
// =========================================================================================
// 🏠 [6] หน้าหลัก StudentActivityPage
// =========================================================================================
function StudentActivityPage() {
  ...
  return (
    // 🛑 สังเกต id="activity-page-root" จุดนี้แหละที่ทำหน้าที่เป็นเป้าหมายให้ Modal ยิงเลเซอร์มาวัดระยะกว้าง/ยาวครับ!
    <div id="activity-page-root" className="min-h-screen relative z-0 pb-10 font-sans px-1"> 
      
      <h1 className="text-2xl sm:text-3xl font-bold text-[#1F384C] mb-6 sm:mb-8">My Activities</h1>
